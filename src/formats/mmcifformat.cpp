...
namespace OpenBabel
{
  ...
  struct CIFTagID
    ...
    enum CIFDataName
      ...
      _atom_site_label_entity_id, // The chain number of the residue, e.g. 2
          ...
          case CIFTagID::atom_site:
            ...
            while (token.type == CIFLexer::ValueToken) // Read in the Fields
              ...
              case CIFTagID::_atom_site_label_comp_id: // The residue abbreviation, e.g. ILE
                ...
                break;
              case CIFTagID::_atom_site_label_entity_id: // The chain number of the residue, e.g. 2
                chain_num = token.as_unsigned();
                break;
  ...
  bool mmCIFFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    if (id.empty())
      {
      snprintf(buffer, BUFF_SIZE, "T%d", static_cast<int>(time(0)));
      id.assign(buffer);
      }
    ofs << "# --------------------------------------------------------------------------" << endl;
    ofs << "#" << endl;
    ofs << "# CIF file generated by openbabel " << BABEL_VERSION << " http://openbabel.sourceforge.net/" << endl;
    ...
    if (pmol->HasData(OBGenericDataType::UnitCell))
      ...
      if (pSG)
        ...
        if (! (pSG->GetHMName().empty()))
          {
          loop_transforms = true;
          ofs << "_space_group.name_H-M_alt\t'" << pSG->GetHMName() << "'" << endl;
          }
        ofs << endl;
